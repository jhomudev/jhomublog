      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5">
        {
          categories?.slice(0, 9).map(({id, img, name ,slug}) => (
            <li key={id} className="w-full h-[80px] hover:brightness-95 rounded-md" style={{backgroundColor:`${CATEGORIE_COLORS[slug as CatColors]}20`}}>
              <Link className="flex gap-3 items-center justify-center h-full" href={`/categories/${slug}`}>
        ...
        <li className="w-full h-[80px] hover:brightness-95 rounded-md bg-bg_soft dark:bg-bg_soft_dark" >
          <Link className="flex gap-3 items-center justify-center h-full" href={`/categories`}>
            Others
          </Link>
        </li>
      </ul>
