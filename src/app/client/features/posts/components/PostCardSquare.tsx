...
function PostCardSquare({post}: Props) {
  return (
    <article className="relative flex flex-col gap-3 w-full rounded-lg">
      ...
      <div className="relative flex w-full h-56 rounded-lg overflow-hidden">
        <Image src={post.img || DEFAULT_POST_IMG} alt={post.title} fill className="object-cover" />
      ...
      <div className="flex flex-col gap-2">
        <h3 className="font-bold text-lg text-pretty line-clamp-2">{ post.title }</h3>
